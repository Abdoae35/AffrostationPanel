// <auto-generated />
using System;
using Agriculture.DAL.Models;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace Agriculture.DAL.Migrations
{
    [DbContext(typeof(AgriContext))]
    [Migration("20250912121529_ModifiyLocationNameDelete")]
    partial class ModifiyLocationNameDelete
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("Agriculture.DAL.Models.AfforestationAgricultureAchievement", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateOnly>("DateOfPlanted")
                        .HasColumnType("date");

                    b.Property<int>("LocationNameId")
                        .HasColumnType("int");

                    b.Property<int>("LocationTypeId")
                        .HasColumnType("int");

                    b.Property<int>("Number")
                        .HasColumnType("int");

                    b.Property<int>("TreeNameId")
                        .HasColumnType("int");

                    b.Property<int>("TreeTypeId")
                        .HasColumnType("int");

                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("LocationNameId");

                    b.HasIndex("LocationTypeId");

                    b.HasIndex("TreeNameId");

                    b.HasIndex("TreeTypeId");

                    b.HasIndex("UserId");

                    b.ToTable("AfforestationAgricultureAchievements");
                });

            modelBuilder.Entity("Agriculture.DAL.Models.LocationName", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("LocationTypeId")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("LocationTypeId");

                    b.ToTable("LocationNames");
                });

            modelBuilder.Entity("Agriculture.DAL.Models.TreeName", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ScientificName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("TypeId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("TypeId");

                    b.ToTable("TreeNames");
                });

            modelBuilder.Entity("Agriculture.DAL.Models.TreeType", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("TreeTypes");
                });

            modelBuilder.Entity("Agriculture.DAL.Models.TypeOfLocation", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("LocationType")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("TypeOfLocations");
                });

            modelBuilder.Entity("Agriculture.DAL.Models.User", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Password")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Role")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("Users");
                });

            modelBuilder.Entity("Agriculture.DAL.Models.AfforestationAgricultureAchievement", b =>
                {
                    b.HasOne("Agriculture.DAL.Models.LocationName", "LocationName")
                        .WithMany()
                        .HasForeignKey("LocationNameId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Agriculture.DAL.Models.TypeOfLocation", "LocationType")
                        .WithMany()
                        .HasForeignKey("LocationTypeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Agriculture.DAL.Models.TreeName", "TreeName")
                        .WithMany()
                        .HasForeignKey("TreeNameId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Agriculture.DAL.Models.TreeType", "TreeType")
                        .WithMany()
                        .HasForeignKey("TreeTypeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Agriculture.DAL.Models.User", "User")
                        .WithMany("AfforestationAchievements")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("LocationName");

                    b.Navigation("LocationType");

                    b.Navigation("TreeName");

                    b.Navigation("TreeType");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Agriculture.DAL.Models.LocationName", b =>
                {
                    b.HasOne("Agriculture.DAL.Models.TypeOfLocation", "LocationType")
                        .WithMany("LocationNames")
                        .HasForeignKey("LocationTypeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("LocationType");
                });

            modelBuilder.Entity("Agriculture.DAL.Models.TreeName", b =>
                {
                    b.HasOne("Agriculture.DAL.Models.TreeType", "Type")
                        .WithMany("TreeNames")
                        .HasForeignKey("TypeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Type");
                });

            modelBuilder.Entity("Agriculture.DAL.Models.TreeType", b =>
                {
                    b.Navigation("TreeNames");
                });

            modelBuilder.Entity("Agriculture.DAL.Models.TypeOfLocation", b =>
                {
                    b.Navigation("LocationNames");
                });

            modelBuilder.Entity("Agriculture.DAL.Models.User", b =>
                {
                    b.Navigation("AfforestationAchievements");
                });
#pragma warning restore 612, 618
        }
    }
}
