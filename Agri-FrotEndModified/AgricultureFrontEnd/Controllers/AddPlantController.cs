...
public class AddPlantController : Controller
{
    ...
    // POST: Submit form
    [HttpPost]
public async Task<IActionResult> Create([FromBody] AfforestationFormVM form)
{
    ...
    if (!User.IsInRole("Admin"))
    {
        ...
        {
            if (isAjax)
                return Json(new { success = false, message = "لا يمكن اختيار تاريخ في المستقبل." });
        ...
        {
            if (isAjax)
                return Json(new { success = false, message = "يمكنك اختيار تاريخ من آخر أسبوعين فقط." });
    ...
    {
        if (isAjax)
            return Json(new { success = false, message = "يرجى التحقق من جميع الحقول." });
    ...
    {
        if (isAjax)
            return Json(new { success = true, message = "تمت إضافة عملية التشجير بنجاح." });
    ...
    if (isAjax)
        return Json(new { success = false, message = "فشل في إضافة عملية التشجير." });
